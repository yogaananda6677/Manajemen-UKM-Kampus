...
// Buat ID Otomatis
$query = mysqli_query($conn, "SELECT MAX(RIGHT(id_ukm, 4)) AS kode FROM ukm");
$data = mysqli_fetch_assoc($query);
$kodeBaru = $data['kode'] ? (int)$data['kode'] + 1 : 1;
$generatedId = 'UKM' . str_pad($kodeBaru, 4, '0', STR_PAD_LEFT);

// Jika form disubmit
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $id = input($_POST["id_ukm"]);
    $nama = input($_POST["nama_ukm"]);
    ...
    if (mysqli_num_rows($cek) > 0) {
        ...
        $sql = "INSERT INTO ukm (id_ukm, nama_ukm) VALUES ('$id', '$nama')";
...
<!-- Modal Background -->
<div id="tambahData" class="fixed top-0 left-0 w-full h-full bg-black/50 z-50 flex items-center justify-center hidden">

  <!-- Form -->
  <form action="" method="post" class="p-6 rounded-2xl border bg-white space-y-4 max-w-sm w-full shadow-xl">

    <h3 class="text-2xl font-bold text-center">Form Input UKM</h3>

    <!-- ID UKM (disembunyikan) -->
    <input type="" name="id_ukm" readonly value="<?= $generatedId ?>">

    <!-- Nama UKM -->
    <div class="form-control">
      <label for="nama" class="label">Nama UKM</label>
      <input required id="nama" name="nama_ukm" type="text" class="input rounded-xl input-bordered w-full" />
    </div>

    <!-- Tombol Aksi -->
    <button type="submit" class="btn btn-primary w-full rounded-xl">Simpan</button>
    <button type="button" onclick="document.getElementById('tambahData').classList.add('hidden')" class="btn btn-neutral w-full rounded-xl">Batal</button>
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  if (alertSuccess) {
    ...
    if (currentPage !== "hash.php") {
        window.location.href = "ukm.php";
    }
    }, 3050);
  }
